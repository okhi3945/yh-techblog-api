...
resource "aws_subnet" "public_subnet" {
  count             = 2
  vpc_id            = aws_vpc.techblog_vpc.id
  cidr_block        = cidrsubnet(aws_vpc.techblog_vpc.cidr_block, 8, count.index) # 10.0.0.0/24, 10.0.1.0/24
...
resource "aws_subnet" "private_subnet" {
  count             = 2
  vpc_id            = aws_vpc.techblog_vpc.id
  cidr_block        = cidrsubnet(aws_vpc.techblog_vpc.cidr_block, 8, count.index + 2) # 10.0.2.0/24, 10.0.3.0/24
  availability_zone = data.aws_availability_zones.available.names[count.index]
  tags = {
...
resource "aws_route" "public_internet_route" {
  ...
  gateway_id             = aws_internet_gateway.techblog_igw.id
}
...
resource "aws_eip" "techblog_nat_eip" {
  domain = "vpc"
  tags = {
...
resource "aws_nat_gateway" "techblog_nat" {
  allocation_id = aws_eip.techblog_nat_eip.id
